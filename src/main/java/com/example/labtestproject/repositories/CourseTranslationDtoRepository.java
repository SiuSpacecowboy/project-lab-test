...
@Repository
public interface CourseTranslationDtoRepository extends JpaRepository<CourseTranslationEntity, Long> {

    @Transactional
    @Modifying
    @Query("update CourseTranslationEntity c set c.courseDate = ?1, c.courseUsdRub = ?2, c.courseUsdKzt = ?3 where c.id = ?4")
