...
public class AccountService {
    ...
    @Autowired
    public AccountService(AccountDtoRepository repository) {
    ...
    public ResponseEntity<?> findAccountByIdForController(long id) {
        Optional<AccountEntity> acc = repository.findById(id);
        ResponseEntity<?> resp;
        if (acc.isEmpty()) {
    ...
    public synchronized ResponseEntity<String> saveNewBill() {
        ...
        if (maxAcc.isEmpty()) {
            repository.save(new AccountEntity("0000000001"));
        } else {
            repository.save(new AccountEntity(String.format("%010d", maxAcc.get() + 1)));
        }
